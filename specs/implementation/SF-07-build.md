...
| Runs in CI against the fixture dataset with no network | `tests/api/conftest.py` sets `SNAP_USE_FIXTURES=1` and asserts `data/snapshots/` is absent; `tests/api/test_health.py::test_fixture_mode_flag_is_true`, `tests/api/test_predict.py::test_no_outbound_socket` (monkeypatches `socket.socket` to raise for the duration of a `/predict` call) |
